...
if (!exists("TEST") || is.null(TEST)) {
  ...
  # Testing mode
  report_params <- parse_params("/blue/cancercenter-dept/hkates/Tests/rnareport/output/report-with-NOISeq-test_params.txt")
}
```
...
if (exists("de_method") && !is.null(de_method)) {
  method_text <- switch(de_method,
    "deseq2" = paste0(
      ...
      filter_text,
      " DESeq2's negative binomial generalized linear model was used to model count data, estimate gene-wise dispersion parameters, and perform differential expression testing. Log-fold changes were shrunk using apeglm to provide more accurate effect size estimates by reducing noise in low-count genes."
    ),
    "edger_GLM" = paste0(
