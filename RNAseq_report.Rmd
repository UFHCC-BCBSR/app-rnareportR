...
```{r paired_viz_intro, results='asis', echo=FALSE}
# Check if this is a paired design
is_paired <- !is.null(report_params$paired_design) && report_params$paired_design == TRUE

if (is_paired) {
  cat("**Note:** This study uses a paired design. Visualizations include standard DE plots and paired-specific analyses showing individual subject changes.\n\n")
}
```
...
if (is_paired) {
  cat("*For paired data, large fold changes without significance may indicate consistent direction but high inter-subject variability.*\n\n")
}
...
```{r heatmap_intro,results='asis',echo=FALSE}
if (is_paired) {
  cat("#### Standard heatmap (clustered by expression similarity)\n\n")
  cat("Heatmaps of the **top 50 genes by p-value** show scaled expression values. In paired studies, samples may cluster by subject rather than group, which is expected and correct.\n\n")
} else {
  cat("Heatmaps of the **top 50 DE genes** for each contrast show scaled counts values using heatmaply v1.5. Hierarchical clustering of samples is based on expression patterns.\n\n")
}
```

```{r heatmaps_standard,fig.width=9, fig.height=9}
heatmap_list <- lapply(seq_along(efit_results_dfs), function(i) {
  generate_heatmap(
    ...
    title = paste("Scaled Gene Expression (Z-scores of logCPM) of Top 50 Genes\n", gsub("efit_|_results_df","",names(efit_results_dfs)[i])),  
    num_genes = 50,  
    fontsize_row = 9
  )
})
...
```{r subject_heatmaps,eval=is_paired,results='asis',fig.width=10, fig.height=16}
cat("#### Subject-ordered heatmap (paired design)\n\n")
cat("Samples are ordered by subject with pre/post pairs side-by-side (not clustered). This validates the paired study design.\n\n")
heatmap_subject_list <- generate_subject_heatmaps(efit_results_dfs, lcpm_matrix, dge_list_filt, num_genes = 50)

# Print each heatmap individually
for(i in seq_along(heatmap_subject_list)) {
  print(heatmap_subject_list[[i]])
}
```

```{r delta_plots,eval=is_paired,results='asis',fig.width=10, fig.height=8}
cat("### Delta plots - individual subject changes\n\n")
cat("Each point shows the change (post - pre) for one subject. Consistent direction across subjects indicates a reproducible effect, even if not statistically significant with current sample size.\n\n")
delta_plot_list <- generate_delta_plots(efit_results_dfs, lcpm_matrix, dge_list_filt, top_n = 10)

# Print each plot individually
for(i in seq_along(delta_plot_list)) {
  print(delta_plot_list[[i]])
  cat("\n\n")  # Add spacing between plots
}
```

```{r trajectory_plots,eval=is_paired,results='asis',fig.width=10, fig.height=8}
cat("### Trajectory plots - pre to post changes\n\n")
cat("Lines connect paired measurements from the same subject. Use the dropdown menu to select different genes (type to search). Parallel lines indicate consistent changes; crossing lines indicate subject-specific responses.\n\n")
trajectory_plot_list <- generate_trajectory_plots(efit_results_dfs, lcpm_matrix, dge_list_filt, max_genes = 100)
tagList(trajectory_plot_list)
```
...
```{r kegg plot, fig.width=12, fig.height=10, eval=!is.null(kegg_results$interactive_plot)}
download_button_png(kegg_results$static_plot, "kegg_enrichment_plot", height=12)
kegg_results$interactive_plot
```

```{r no kegg, eval=is.null(kegg_results$interactive_plot), results='asis'}
cat("No significant KEGG pathways were found for the given significance threshold.\n")
```
